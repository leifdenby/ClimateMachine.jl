...
# - `aux.coord` is available here because we've specified `coord` in `vars_state_auxiliary`.
...
# - this method is only called at `t=0`.
# - `state.ρ`, `state.ρu` and`state.ρcT` are available here because we've specified `ρ`, `ρu` and `ρcT` in `vars_state_conservative`.
...
# e.g. `transform.ρcT` is available here because we've specified `ρcT` in `vars_state_gradient`.
...
#  - `diffusive.μ∇u` is available here because we've specified `μ∇u` in `vars_state_gradient_flux`.
#  - `∇transform.u` is available here because we've specified `u` in `vars_state_gradient`.
...
# Note that:
# - `state.ρu` is available here because we've specified `ρu` in `vars_state_conservative`.

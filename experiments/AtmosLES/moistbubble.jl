...
function init_drybubble!(bl, state, aux, (x, y, z), t)
    ...
    ## Initial condition, known profiles
    q_tot_0 = FT(0.01) ## TODO : populate with correct profile
    θ_liq = #known
    Pressure = #can calculate 
    
    ## Use wrappers below to compute thermodynamic state object TS, 
    #then compute all required parameters as function of TS
    ...
    ## Compute other thermodynamic parameters
    ρ = air_density(TS)
    ...
    state.ρu = ρu
    state.ρe = ρe_tot
    state.moisture.ρq_tot = ## TODO: assign correct initial moisture profile

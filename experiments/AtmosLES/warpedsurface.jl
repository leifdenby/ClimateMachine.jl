...
function warp_schar(xin, yin, zin)
  ...
  r² = (xin-xc)^2 + (yin-yc)^2 
  λ = FT(2500)
  zdiff = FT(500) * exp(-(xin-xc)/λ) * (cos(π * (xin-xc) / λ))^2
  # Linear relaxation towards domain maximum height
  x, y, z = xin, yin, zin + zdiff * (zmax- zin)/zmax
end

function warp_agnesi(xin, yin, zin, xmax, ymax, zmax)
  FT = eltype(xin)
  a = FT(500)
  xc = FT(1/2) * xmax
  yc = FT(1/2) * ymax
...
function config_schar(FT, N, resolution, xmax, ymax, zmax)
    ...
    warp_agnesi(xin,yin,zin,_...) = warp_agnesi(xin,yin,zin,xmax,ymax,zmax)

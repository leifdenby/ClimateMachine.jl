...
function kinematic_model_nodal_update_auxiliary_state!(
    ...
    aux.src_snow_melt = snow_melt(param_set, snow_param_set, aux.q_sno, state.ρ)
...
function source!(
    ...
    evap =
        ρ *
        evaporation_sublimation(param_set, rain_param_set, q, q_rai, state.ρ, T)
    source.ρq_rai += evap
    source.ρq_tot -= evap

    # snow -> vapour
    subl =
        evaporation_sublimation(param_set, snow_param_set, q, q_sno, state.ρ, T)
    source.ρq_sno += subl
    source.ρq_tot -= subl
    ...
    # TODO - do not export some of the "internal" microphysics functions
    #        (like unpack_params)

...
struct HydrostaticBalanceModel <: BalanceLaw end

vars_state_auxiliary(::HydrostaticBalanceModel, T) = @vars(p::T)
vars_state_conservative(::HydrostaticBalanceModel, T) = @vars(∇p::SVector{3, T})
vars_state_gradient(::HydrostaticBalanceModel, T) = @vars()
vars_state_gradient_flux(::HydrostaticBalanceModel, T) = @vars()

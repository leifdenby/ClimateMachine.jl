...
# A NamedTuple of all `number_states`'s.
all_number_states(balance_law) = (
    prognostic = number_states(balance_law, Prognostic()),
    auxiliary = number_states(balance_law, Auxiliary()),
    gradient = number_states(balance_law, Gradient()),
    grad_flux = number_states(balance_law, GradientFlux()),
    grad_lap = number_states(balance_law, GradientLaplacian()),
    up_integrals = number_states(balance_law, UpwardIntegrals()),
    dn_integrals = number_states(balance_law, DownwardIntegrals()),
    hyperdiff = number_states(balance_law, Hyperdiffusive()),
)
...
@kernel function volume_tendency!(
    ...
    @uniform begin
        ...
        ns = all_number_states(balance_law)

        Nq = dofs_per_elem_per_direction(N)
        Nqk = dofs_per_elem_per_reduced_direction(N, dim)

        local_source = MArray{Tuple{ns.prognostic}, FT}(undef)
        local_state_prognostic = MArray{Tuple{ns.prognostic}, FT}(undef)
        local_state_gradient_flux = MArray{Tuple{ns.grad_flux}, FT}(undef)
        local_state_hyperdiffusion = MArray{Tuple{ns.hyperdiff}, FT}(undef)
        local_state_auxiliary = MArray{Tuple{ns.auxiliary}, FT}(undef)
        local_flux = MArray{Tuple{3, ns.prognostic}, FT}(undef)
        local_flux_3 = MArray{Tuple{ns.prognostic}, FT}(undef)
    end

    shared_flux = @localmem FT (2, Nq, Nq, ns.prognostic)
    s_D = @localmem FT (Nq, Nq)

    local_tendency = @private FT (Nqk, ns.prognostic)
    ...
    @inbounds begin
        ...
        @unroll for k in 1:Nqk
            ijk = i + Nq * ((j - 1) + Nq * (k - 1))
            @unroll for s in 1:(ns.prognostic)
...
@kernel function volume_tendency!(
    ...
    @uniform begin
        ...
        local_state_auxiliary = MArray{Tuple{ns.auxiliary}, FT}(undef)
        local_flux = MArray{Tuple{3, ns.prognostic}, FT}(undef)
        local_flux_total = MArray{Tuple{3, ns.prognostic}, FT}(undef)

        _ζx1 = dim == 2 ? _ξ2x1 : _ξ3x1
        _ζx2 = dim == 2 ? _ξ2x2 : _ξ3x2
        _ζx3 = dim == 2 ? _ξ2x3 : _ξ3x3

        shared_flux_size = dim == 2 ? (Nq, Nq, ns.prognostic) : (0, 0, 0)
    end
...
@kernel function interface_tendency!(
    ...
    @uniform begin
        ...
        local_state_prognostic⁺nondiff = MArray{Tuple{ns.prognostic}, FT}(undef)
        local_state_prognostic⁺diff = MArray{Tuple{ns.prognostic}, FT}(undef)

        # Need two copies since numerical_flux_first_order! can modify state_auxiliary⁺
        local_state_auxiliary⁺nondiff = MArray{Tuple{ns.auxiliary}, FT}(undef)
        local_state_auxiliary⁺diff = MArray{Tuple{ns.auxiliary}, FT}(undef)

        local_state_gradient_flux⁺ = MArray{Tuple{ns.grad_flux}, FT}(undef)
        local_state_hyperdiffusion⁺ = MArray{Tuple{ns.hyperdiff}, FT}(undef)

        local_state_prognostic_bottom1 = MArray{Tuple{ns.prognostic}, FT}(undef)
        local_state_gradient_flux_bottom1 =
            MArray{Tuple{ns.grad_flux}, FT}(undef)
        local_state_auxiliary_bottom1 = MArray{Tuple{ns.auxiliary}, FT}(undef)

        local_flux = MArray{Tuple{ns.prognostic}, FT}(undef)
    end
...
@kernel function volume_gradients!(
    ...
    @uniform begin
        ...
        local_transform = MArray{Tuple{ns.gradient}, FT}(undef)
        local_state_gradient_flux = MArray{Tuple{ns.grad_flux}, FT}(undef)
    end

    shared_transform = @localmem FT (Nq, Nq, ns.gradient)
    s_D = @localmem FT (Nq, Nq)

    local_state_prognostic = @private FT (ns.prognostic, Nqk)
    local_state_auxiliary = @private FT (ns.auxiliary, Nqk)
    local_transform_gradient = @private FT (3, ns.gradient, Nqk)
    Gξ3 = @private FT (ns.gradient, Nqk)
...
@kernel function volume_gradients!(
    ...
    @uniform begin
        ...
        Gζ_size = dim == 3 ? (ns.gradient, Nqk) : (0, 0)
    end
...
@kernel function interface_gradients!(
    ...
    @uniform begin
        ...
        local_state_auxiliary⁻ = MArray{Tuple{ns.auxiliary}, FT}(undef)
        local_transform⁻ = MArray{Tuple{ns.gradient}, FT}(undef)
        l_nG⁻ = MArray{Tuple{3, ns.gradient}, FT}(undef)
        ...
        local_state_gradient_flux = MArray{Tuple{ns.grad_flux}, FT}(undef)
        local_transform_gradient = MArray{Tuple{3, ns.gradient}, FT}(undef)
        local_state_prognostic⁻visc = MArray{Tuple{ns.grad_flux}, FT}(undef)
...
@kernel function kernel_init_state_prognostic!(
    ...
    l_state = MArray{Tuple{ns.prognostic}, FT}(undef)
    local_state_auxiliary = MArray{Tuple{ns.auxiliary}, FT}(undef)
...
@kernel function kernel_indefinite_stack_integral!(
    ...
    @uniform begin
        ...
        Nq = dofs_per_elem_per_direction(N)
        Nqj = dofs_per_elem_per_reduced_direction(N, dim)
...
@kernel function volume_divergence_of_gradients!(
    ...
    @uniform begin
        ...
        l_div = MArray{Tuple{ns.grad_lap}, FT}(undef)
    end

    s_grad = @localmem FT (Nq, Nq, Nqk, ns.grad_lap, 3)
    s_D = @localmem FT (Nq, Nq)
...
@kernel function interface_divergence_of_gradients!(
    ...
    @uniform begin
        ...
        l_grad⁻ = MArray{Tuple{3, ns.grad_lap}, FT}(undef)
        l_grad⁺ = MArray{Tuple{3, ns.grad_lap}, FT}(undef)
        l_div = MArray{Tuple{ns.grad_lap}, FT}(undef)
    end
...
@kernel function volume_gradients_of_laplacians!(
    ...
    @uniform begin
        ...
        l_grad_lap = MArray{Tuple{3, ns.grad_lap}, FT}(undef)
        local_state_hyperdiffusion = MArray{Tuple{ns.hyperdiff}, FT}(undef)
    end

    s_lap = @localmem FT (Nq, Nq, Nqk, ns.grad_lap)
    s_D = @localmem FT (Nq, Nq)
    s_ω = @localmem FT (Nq,)
    local_state_prognostic = @private FT (ns.prognostic,)
    local_state_auxiliary = @private FT (ns.auxiliary,)
...
@kernel function interface_gradients_of_laplacians!(
    ...
    @uniform begin
        ...
        l_lap⁻ = MArray{Tuple{ns.grad_lap}, FT}(undef)
        l_lap⁺ = MArray{Tuple{ns.grad_lap}, FT}(undef)
        local_state_hyperdiffusion = MArray{Tuple{ns.hyperdiff}, FT}(undef)

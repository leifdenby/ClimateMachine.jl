...
"""
Base.@kwdef struct waterfunctions{FT, HC} <: AbstractWater{FT}
    hydraulic_model::HC = vanGenuchten{FT}()
    #matric_pot::MP = vanGenuchten{FT}()
end

function waterfunctions(;
    hydraulic_model::AHM{FT} = vanGenuchten{FT}()
    ) where {FT}
    return waterfunctions{FT,
        typeof(hydraulic_model)
        }(hydraulic_model)
end
